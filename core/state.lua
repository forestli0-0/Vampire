...
--- state.initChapterMode: 初始化章节探索模式
-- 生成线性程序化地下城，使用无感刷怪系统
function state.initChapterMode(opts)
    opts = opts or {}
    ...
    -- 设置运行模式为章节模式
    state.runMode = 'chapter'
    
    -- 生成章节地图
    local chapter = require('world.chapter')
    local chapterMap = chapter.generate({
        nodeCount = opts.nodeCount or 8,
        mapWidth = opts.mapWidth or 200,
        mapHeight = opts.mapHeight or 80,
    })
    ...
    -- 初始化 world 用于碰撞检测 (复用 chapterMap 的 tiles)
    local worldMod = require('world.world')
    state.world = worldMod.new()
