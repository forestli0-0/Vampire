...
## 2025-12-14：确立“单武器 + 船员小队”变种方向（已转向宠物）
...
### MVP 落地（当时的实现，现已替换）
- 船员系统脚手架（已移除，改为宠物）：`crew.lua`
  - 跟随/驻守切换、倒地救援等机制验证通过，但会把系统自然推向“多武器平台＝弹幕天堂”。
- 房间流新增类型：`shop` / `event`
  - 当时用于承载招募交互（`crew_contract`），现已替换为宠物相关交互。
...
## 2025-12-15：从“船员”转向“宠物系统”

### 核心原因（更贴合我们的资源与目标）
- 船员很容易退化成“多武器平台”，把构筑重心再次拉回弹幕天堂；AI/协同设计也会迅速膨胀。
- 宠物更适合做成 **功能型/低频触发**：提供 utility、状态触发、节奏加速与新爽点，同时不吃美术/动作系统红利。
- 宠物天然适配“局外配置 + 局内临时成长”的双轨培养，也更容易围绕倒地/退场/复活事件做肉鸽叙事与风险决策。
...
### 下一步（把“宠物”变成真正的路线）
- 宠物模块扩展：每只宠物至少 2–3 个模块分支（控制/增益/资源/破盾/剥甲），但保持“低频、非武器平台”原则。
- 宠物培养节奏：明确本局宠物成长来源（房间结算/事件/商店），避免与升级池重叠、避免奖励溢出。
- 事件库：围绕“复活/改造/换模块/交易”做可复用事件模板，形成跑内叙事与选择权。 
