...
- 当前实现里存在“模块（局外选择）”：例如 `default/pulse`、`default/field`、`default/barrier`（`arsenal.lua`）。**计划调整为：模块改为“圣物式掉落/商店购买”的局内拾取物，且局内不可替换**（见 4.1）。
